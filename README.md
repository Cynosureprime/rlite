...
## Advanced Options
```
rlite input.txt -S 2G -o output.txt
```
Sort the input.txt file and limit the RAM usage to 2 Gigabytes. Rlite will use best effort to clamp the memory usage and swap to disk.  Specify a dir to use a swap dir using -T otherwise tha current directory will be used.

```
rlite input.txt -S -5 -o output.txt
```
Sort the input.txt file and process the file in 5 parts on disk. Ensure you have adequate memory to hold a single part in memory, accounting for overhead.

```
rlite input.txt -z 4 -o output.txt
```
Sort the input.txt file by similarity using a min-hash shingle size of 4. Increase the shingle size causes more specific matches, decreasing will result in broader matches.

```
rlite largefile.txt search.txt -z 4 -o output.txt
```
Uses similarity matching and write all files from largefile.txt that are similar to those in search.txt with a min-hash shingle size of 4. Increase for more specific matches.
...
* Similarity matching with min/hash shingles. Thanks @chick3nman
* Interleaved reading via double buffering for certain operations
* Added tournament tree for k-way merging when doing multi-part processing
...
This tool is not meant to replace the amazing rling, it is merely an alternative with reduced capability/flexbility. While the tools perform similar functions and share some code, they use a very different implementations. One tool may outperform the other due to hardware specs, list size, mode of operation and operating environment.
